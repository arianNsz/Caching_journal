...
frequency = np.zeros(shape=(3, CAP))
total_mem = 3 * CAP
avg_fresh= 0
current_step = 0
done = False
...
while current_step<= MAX_STEPS:
    ...
    frequency[expired[0], expired[1]]=0
    ...
    if (edge1_has[0].shape != (0,)):
        e1 = 1
        frequency[0, edge1_has[0][0]] += 1
    if (edge2_has[0].shape != (0,)):
        e2 = 1
        frequency[1, edge2_has[0][0]] += 1
    if (parent_has[0].shape != (0,)):
        p_has = 1
        frequency[2, parent_has[0][0]] += 1
    ...
    if do_cache:
        ...
        if empty_e[0].shape != (0,) and which_BS==0:
            ...
            frequency[0, empty_e[0][-1]] = 1
        elif which_BS==0:
            e1_least_frequent= np.argsort(frequency[0, :])[0]
            mem_status[0, e1_least_frequent] = request[0]
            mem_status[1, e1_least_frequent] = 0
            freshness[0, e1_least_frequent] = current_step
            freshness[1, e1_least_frequent] = request[1]
            frequency[0, e1_least_frequent] = 1
        ...
        if empty_e2[0].shape != (0,) and which_BS==1:
            ...
            frequency[1, empty_e2[0][-1]] = 1
        elif which_BS==1:
            e2_least_frequent= np.argsort(frequency[1,:])[0]
            mem_status[2, e2_least_frequent] = request[0]
            mem_status[3, e2_least_frequent] = 0
            freshness[2, e2_least_frequent] = current_step
            freshness[3, e2_least_frequent] = request[1]
            frequency[1, e2_least_frequent] = 1
        ...
        if empty_p[0].shape != (0,) :
            ...
            frequency[2, empty_p[0][-1]] = 1
        else:
            p_least_frequent = np.argsort(frequency[2,:])[0]
            mem_status[4, p_least_frequent] = request[0]
            mem_status[5, p_least_frequent] = 0
            freshness[4, p_least_frequent] = current_step
            freshness[5, p_least_frequent] = request[1]
            frequency[2, p_least_frequent] = 1
    else:
        hits.append(1)
...
plt.title('accumulated rewards LFU')
plt.plot(rewards)
plt.show()
